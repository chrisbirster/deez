...
/// Activate only after a successful dry-run. Target state is reconstructed by
/// replaying immutable history under the target parameters; internal memory
/// state is never copied from the source scheduler.
pub fn activateFsrs7(
    ...
    try store.setDeckFsrs7(deck_id, target_parameter_set_id);
    const study = Study.init(store);
    var rebuilt: usize = 0;
    for (preview.cards) |change| {
        _ = try study.rebuildCardState(allocator, change.card_id, now_ms);
        rebuilt += 1;
    }
    ...
    return .{
        ...
        .rebuilt_cards = rebuilt,
    };
}

test "scheduler migration preview is side effect free and activation replays history" {
