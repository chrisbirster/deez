...
fn promptOwned(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, label: []const u8) ![]u8 {
    try out.writeAll(label);
    try out.flush();
    return readLine(allocator, io);
}

fn promptYesNo(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, label: []const u8, default_yes: bool) !bool {
    while (true) {
...
fn chooseDeck(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, store: *storage.Store, requested: ?u64) !u64 {
    ...
    const decks = try store.decks(allocator, nowMs(io));
    defer {
...
fn noteBelongsToDeck(allocator: std.mem.Allocator, store: *storage.Store, deck_id: u64, note_id: content.NoteId) !bool {
...
fn chooseNote(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, store: *storage.Store, deck_id: u64, requested: ?content.NoteId) !content.NoteId {
...
fn parseChoices(allocator: std.mem.Allocator, source: []const u8) ![]Choice {
    ...
    for (parsed.value, 0..) |choice, index| {
        result[index] = .{ .id = try allocator.dupe(u8, choice.id), .text = try allocator.dupe(u8, choice.text) };
        completed += 1;
    }
    return result;
}
...
fn editChoiceTexts(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, choices: []Choice) !void {
    try out.writeAll("\nChoices:\n");
    for (choices) |*choice| {
...
fn editTwoField(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote) ![][]u8 {
...
fn editCloze(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote) ![][]u8 {
...
fn editChoiceNote(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote, multiple: bool) ![][]u8 {
    ...
    if (!multiple) {
        ...
        try out.print("\nCurrent correct choices: {s}\n", .{try fieldAt(note, 2)});
        while (true) {
            const line = try promptOwned(allocator, io, out, "Correct choices (comma separated; Enter = keep): ");
            if (trimmed(line).len == 0) {
                allocator.free(line);
                correct = try allocator.dupe(u8, try fieldAt(note, 2));
                break;
            }
            defer allocator.free(line);
...
fn editOrdering(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote) ![][]u8 {
    ...
    try out.writeAll("\nItems remain in canonical order; edit their text below.\n");
...
fn editImageOcclusion(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote) ![][]u8 {
    const fields = try allocator.alloc([]u8, 3);
    errdefer allocator.free(fields);
...
fn editLegacyOptionalReverse(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote) ![][]u8 {
    try out.writeAll("\nThis is a legacy optional-reverse note. Its existing behavior will be preserved.\n");
    const fields = try allocator.alloc([]u8, 3);
    errdefer allocator.free(fields);
    fields[0] = try promptExisting(allocator, io, out, "Front", try fieldAt(note, 0), false);
    errdefer allocator.free(fields[0]);
...
fn editFields(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, note: content.OwnedNote, kind: content.BuiltInNoteType) ![][]u8 {
    return switch (kind) {
        .basic, .basic_reverse, .type_answer => editTwoField(allocator, io, out, note),
...
fn freeFields(allocator: std.mem.Allocator, fields: [][]u8) void {
    for (fields) |field| allocator.free(field);
    allocator.free(fields);
}

fn preview(allocator: std.mem.Allocator, out: *Io.Writer, kind: content.BuiltInNoteType, fields: []const []const u8) !void {
    ...
    for (rendered, 0..) |card, index| try out.print("\nCard {d}\nFront:\n{s}\n\nBack:\n{s}\n", .{
        index + 1,
        card.rendered.front,
        card.rendered.back,
    });
...
fn runWithStore(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, store: *storage.Store, request: Request) !void {
    const deck_id = try chooseDeck(allocator, io, out, store, request.deck_id);
    while (true) {
        ...
        if (try promptYesNo(allocator, io, out, "\nSave changes? [Y/n] ", true)) {
            const ids = try note_mutation.update(allocator, store, deck_id, note_id, fields, note.tags_json, nowMs(io));
            defer allocator.free(ids);
            try out.print("Updated note {d} ({d} active card{s}).\n", .{ note_id, ids.len, if (ids.len == 1) "" else "s" });
        } else {
...
test "interactive edit does not steal scripted note edit" {
    const edit = [_][]const u8{ "deez", "edit" };
    const edit_note = [_][]const u8{ "deez", "edit", "4", "9" };
    const note_edit = [_][]const u8{ "deez", "note", "edit", "4", "9" };
    const scripted = [_][]const u8{ "deez", "note", "edit", "4", "9", "front", "back" };
