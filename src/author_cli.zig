...
fn promptOwned(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, label: []const u8, allow_empty: bool) ![]u8 {
    while (true) {
...
fn chooseDeck(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, store: *storage.Store, requested: ?u64, now_ms: i64) !u64 {
...
fn chooseNoteType(allocator: std.mem.Allocator, io: Io, out: *Io.Writer) !content.BuiltInNoteType {
    try out.writeAll("\nNote type:\n");
    for (authorable_note_types, 0..) |kind, index| try out.print("  {d}. {s}\n", .{ index + 1, noteTypeName(kind) });
    while (true) {
...
fn twoFieldDraft(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, kind: content.BuiltInNoteType) !Draft {
...
fn clozeDraft(allocator: std.mem.Allocator, io: Io, out: *Io.Writer) !Draft {
    try out.writeAll("Use {{c1::answer}} syntax directly. Matching cloze numbers hide together.\n");
    const fields = try allocator.alloc([]u8, 2);
    errdefer allocator.free(fields);
    fields[0] = try promptOwned(allocator, io, out, "Text: ", false);
    errdefer allocator.free(fields[0]);
    fields[1] = try promptOwned(allocator, io, out, "Extra information (optional): ", true);
...
fn collectChoices(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, item_name: []const u8) ![]Choice {
...
fn choiceDraft(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, multiple: bool) !Draft {
    ...
    if (!multiple) {
        ...
        while (true) {
            ...
            while (parts.next()) |part| {
                ...
                for (ids.items) |existing| if (std.mem.eql(u8, existing, choices[index].id)) {
                    valid = false;
                    break;
                };
                if (!valid) break;
...
fn buildDraft(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, kind: content.BuiltInNoteType) !Draft {
    return switch (kind) {
...
fn previewDraft(allocator: std.mem.Allocator, out: *Io.Writer, draft: Draft) !void {
...
fn runWithStore(allocator: std.mem.Allocator, io: Io, out: *Io.Writer, store: *storage.Store, requested_deck_id: ?u64) !void {
    const deck_id = try chooseDeck(allocator, io, out, store, requested_deck_id, nowMs(io));
    while (true) {
        ...
        if (try promptYesNo(allocator, io, out, "\nSave this note? [Y/n] ", true)) {
            const result = try note_mutation.create(allocator, store, deck_id, draft.kind, draft.fields, "[]", nowMs(io));
...
test "interactive authoring does not steal scripted note add" {
    const add = [_][]const u8{ "deez", "add" };
    const add_deck = [_][]const u8{ "deez", "add", "4" };
    const note_add = [_][]const u8{ "deez", "note", "add" };
    const note_add_deck = [_][]const u8{ "deez", "note", "add", "4" };
    const scripted = [_][]const u8{ "deez", "note", "add", "4", "basic", "front", "back" };
