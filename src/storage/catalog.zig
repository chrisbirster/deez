...
pub const Catalog = struct {
    ...
    fn bindBlob32(stmt: *c.sqlite3_stmt, index: c_int, id: *const fsrs.ParameterSetId) !void {
        if (c.sqlite3_bind_blob(stmt, index, id, 32, null) != c.SQLITE_OK) return error.SqliteBindFailed;
    }
    ...
    fn columnBlob32(stmt: *c.sqlite3_stmt, column: c_int) !fsrs.ParameterSetId {
        ...
        const opaque = c.sqlite3_column_blob(stmt, column) orelse return error.InvalidParameterSetId;
        const bytes: [*]const u8 = @ptrCast(opaque);
    ...
    pub fn parameterSetId(parameters: fsrs.v7.Parameters) fsrs.ParameterSetId {
        ...
        for (parameters.weights, 0..) |weight, index| {
            ...
            for (&lanes, 0..) |*lane, lane_index| {
                mix(lane, bits ^ (@as(u64, @intCast(index + 1)) *% (@as(u64, lane_index + 1) *% 0x517cc1b727220a95)));
            }
        }
    ...
    pub fn resolveDeckScheduler(self: Catalog, deck_id: card_mod.DeckId, now_ms: time.TimestampMs) !ResolvedScheduler {
        ...
        const major: u16 = @intCast(c.sqlite3_column_int(stmt, 1));
        const parameter_set_id = try columnBlob32(stmt, 2);
        return .{
            .algorithm = .{ .family = .fsrs, .major = major },
...
test "deck parameter scope overrides group and global" {
    ...
    try db.migrate();
    const catalog: Catalog = .{ .db = &db };
    const global_id = try catalog.ensureDefaultFsrs7(0);
    ...
    try std.testing.expect(!std.mem.eql(u8, &global_id, &deck_parameter_id));
}
