...
pub const Catalog = struct {
    ...
    fn bindBlob32(stmt: *c.sqlite3_stmt, index: c_int, id: *const fsrs.ParameterSetId) !void {
        if (c.sqlite3_bind_blob(stmt, index, @ptrCast(id), 32, null) != c.SQLITE_OK) return error.SqliteBindFailed;
    }
    ...
    pub fn putFsrs7Parameters(
        ...
        const weight_stmt = try self.prepare("INSERT INTO parameter_weights(parameter_set_id, position, value) VALUES (?1, ?2, ?3);");
    ...
    pub fn loadFsrs7Parameters(self: Catalog, id: fsrs.ParameterSetId) !fsrs.v7.Parameters {
        const set_stmt = try self.prepare("SELECT algorithm_family, algorithm_major, desired_retention, minimum_interval_days, maximum_interval_days FROM parameter_sets WHERE id = ?1;");
        defer _ = c.sqlite3_finalize(set_stmt);
        var stable_id = id;
        ...
        const weight_stmt = try self.prepare("SELECT position, value FROM parameter_weights WHERE parameter_set_id = ?1 ORDER BY position;");
        defer _ = c.sqlite3_finalize(weight_stmt);
    ...
    pub fn ensureDefaultFsrs7(self: Catalog, created_at_ms: time.TimestampMs) !fsrs.ParameterSetId {
        ...
        const stmt = try self.prepare("UPDATE scheduler_defaults SET algorithm_family = 'fsrs', algorithm_major = 7, parameter_set_id = ?1 WHERE id = 1 AND parameter_set_id IS NULL;");
        defer _ = c.sqlite3_finalize(stmt);
        var stable_id = id;
        try bindBlob32(stmt, 1, &stable_id);
        try stepDone(stmt);
    ...
    pub fn resolveDeckScheduler(self: Catalog, deck_id: card_mod.DeckId, now_ms: time.TimestampMs) !ResolvedScheduler {
        _ = try self.ensureDefaultFsrs7(now_ms);
        const stmt = try self.prepare("SELECT COALESCE(d.algorithm_family, g.algorithm_family, s.algorithm_family), COALESCE(d.algorithm_major, g.algorithm_major, s.algorithm_major), COALESCE(d.parameter_set_id, g.parameter_set_id, s.parameter_set_id) FROM decks d LEFT JOIN deck_groups g ON g.id = d.group_id CROSS JOIN scheduler_defaults s WHERE d.id = ?1 AND s.id = 1;");
        defer _ = c.sqlite3_finalize(stmt);
        ...
        return .{
            .algorithm = .{ .family = .fsrs, .major = @intCast(c.sqlite3_column_int(stmt, 1)) },
    ...
    pub fn setGlobalFsrs7(self: Catalog, parameter_set_id: fsrs.ParameterSetId) !void {
        ...
        const stmt = try self.prepare("UPDATE scheduler_defaults SET algorithm_family = 'fsrs', algorithm_major = 7, parameter_set_id = ?1 WHERE id = 1;");
        defer _ = c.sqlite3_finalize(stmt);
    ...
    pub fn createGroup(self: Catalog, name: []const u8, created_at_ms: time.TimestampMs) !u64 {
        ...
        try stepDone(stmt);
        return @intCast(c.sqlite3_last_insert_rowid(self.db.handle));
    }
    ...
    pub fn assignDeckGroup(self: Catalog, deck_id: card_mod.DeckId, group_id: ?u64) !void {
        ...
        } else if (c.sqlite3_bind_null(stmt, 1) != c.SQLITE_OK) return error.SqliteBindFailed;
        try bindId(stmt, 2, deck_id);
        try stepDone(stmt);
    }

    pub fn inheritDeckScheduler(self: Catalog, deck_id: card_mod.DeckId) !void {
        const stmt = try self.prepare("UPDATE decks SET algorithm_family = NULL, algorithm_major = NULL, parameter_set_id = NULL WHERE id = ?1;");
        defer _ = c.sqlite3_finalize(stmt);
        try bindId(stmt, 1, deck_id);
        try stepDone(stmt);
    }
    ...
    pub fn setGroupFsrs7(self: Catalog, group_id: u64, parameter_set_id: fsrs.ParameterSetId) !void {
        _ = try self.loadFsrs7Parameters(parameter_set_id);
        const stmt = try self.prepare("UPDATE deck_groups SET algorithm_family = 'fsrs', algorithm_major = 7, parameter_set_id = ?1 WHERE id = ?2;");
        defer _ = c.sqlite3_finalize(stmt);
        var stable_id = parameter_set_id;
        try bindBlob32(stmt, 1, &stable_id);
    ...
    pub fn setDeckFsrs7(self: Catalog, deck_id: card_mod.DeckId, parameter_set_id: fsrs.ParameterSetId) !void {
        _ = try self.loadFsrs7Parameters(parameter_set_id);
        const stmt = try self.prepare("UPDATE decks SET algorithm_family = 'fsrs', algorithm_major = 7, parameter_set_id = ?1 WHERE id = ?2;");
        defer _ = c.sqlite3_finalize(stmt);
        var stable_id = parameter_set_id;
        try bindBlob32(stmt, 1, &stable_id);
        try bindId(stmt, 2, deck_id);
    ...
    pub fn dueCards(self: Catalog, allocator: std.mem.Allocator, deck_id: card_mod.DeckId, now_ms: time.TimestampMs, limit: usize) ![]OwnedDueCard {
        const stmt = try self.prepare("SELECT c.id, c.deck_id, c.question, c.answer, s.due_at_ms FROM cards c LEFT JOIN scheduler_state s ON s.card_id = c.id WHERE c.deck_id = ?1 AND (s.card_id IS NULL OR s.due_at_ms <= ?2) ORDER BY COALESCE(s.due_at_ms, c.created_at_ms), c.id LIMIT ?3;");
        defer _ = c.sqlite3_finalize(stmt);
        try bindId(stmt, 1, deck_id);
            ...
            switch (c.sqlite3_step(stmt)) {
                c.SQLITE_ROW => {
                    ...
                    try cards.append(allocator, .{
                        ...
                        .question = question,
                        .answer = answer,
                        .due_at_ms = if (c.sqlite3_column_type(stmt, 4) == c.SQLITE_NULL) null else c.sqlite3_column_int64(stmt, 4),
                    });
                },
                c.SQLITE_DONE => break,
                else => return error.SqliteStepFailed,
    ...
    pub fn appendReview(self: Catalog, card_id: card_mod.CardId, rating: fsrs.Rating, reviewed_at_ms: time.TimestampMs, stamp: fsrs.SchedulerStamp, scheduled_at_ms: time.TimestampMs) !u64 {
        if (stamp.algorithm.family != .fsrs) return error.UnsupportedAlgorithmFamily;
        const stmt = try self.prepare("INSERT INTO reviews(card_id, rating, reviewed_at_ms, algorithm_family, algorithm_major, implementation_major, implementation_minor, implementation_patch, parameter_set_id, scheduled_at_ms) VALUES (?1, ?2, ?3, 'fsrs', ?4, ?5, ?6, ?7, ?8, ?9);");
        defer _ = c.sqlite3_finalize(stmt);
    ...
    pub fn getSchedulerState(self: Catalog, card_id: card_mod.CardId) !?SchedulerState {
        const stmt = try self.prepare("SELECT algorithm_major, implementation_major, implementation_minor, implementation_patch, parameter_set_id, stability_days, difficulty, due_at_ms, last_reviewed_at_ms FROM scheduler_state WHERE card_id = ?1;");
        defer _ = c.sqlite3_finalize(stmt);
        try bindId(stmt, 1, card_id);
        ...
        return .{
            .card_id = card_id,
            .stamp = .{
                .algorithm = .{ .family = .fsrs, .major = @intCast(c.sqlite3_column_int(stmt, 0)) },
                .implementation = .{
            ...
            .stability_days = if (c.sqlite3_column_type(stmt, 5) == c.SQLITE_NULL) null else c.sqlite3_column_double(stmt, 5),
            .difficulty = if (c.sqlite3_column_type(stmt, 6) == c.SQLITE_NULL) null else c.sqlite3_column_double(stmt, 6),
            .due_at_ms = c.sqlite3_column_int64(stmt, 7),
            .last_reviewed_at_ms = if (c.sqlite3_column_type(stmt, 8) == c.SQLITE_NULL) null else c.sqlite3_column_int64(stmt, 8),
        };
    }
};
...
test "parameter sets round trip and resolve globally" {
    ...
    try std.testing.expectEqual(id, parameterSetId(loaded));
...
test "deck parameter scope overrides group" {
    var db = try sqlite.Db.open(":memory:");
    defer db.close();
    try db.migrate();
    const catalog: Catalog = .{ .db = &db };
...
test "due queue includes unscheduled cards" {
    var db = try sqlite.Db.open(":memory:");
    defer db.close();
    try db.migrate();
    const catalog: Catalog = .{ .db = &db };
    ...
    try catalog.upsertSchedulerState(.{
        .card_id = first,
        .stamp = .{ .algorithm = .fsrs7, .implementation = .current, .parameter_set_id = parameter_id },
        .stability_days = 2,
        .difficulty = 4,
        .due_at_ms = 100,
