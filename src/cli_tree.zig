...
fn studyHandler(ctx: *th.Context) !void {
    const order = if (ctx.optionValue("order")) |value| try parseStudyOrder(value) else cli.StudyOrder.due;
    const new_limit = if (ctx.optionValue("new-limit")) |value| try parseCount(value) else null;
...
fn fsrsOptimizeHandler(ctx: *th.Context) !void {
    try setRoute(ctx, .{ .core = .{ .fsrs_optimize = .{
        .deck_id = if (ctx.args.len == 1) try parseId(ctx.args[0]) else null,
        .recency_half_life_days = if (ctx.hasOption("recency")) 1.0 else null,
    } } });
}

fn fsrsEvaluateHandler(ctx: *th.Context) !void {
    try setRoute(ctx, .{ .core = .{ .fsrs_evaluate = .{
        .deck_id = if (ctx.args.len == 1) try parseId(ctx.args[0]) else null,
    } } });
}

fn fsrsSimulateHandler(ctx: *th.Context) !void {
    try setRoute(ctx, .{ .core = .{ .fsrs_simulate = .{
        .retention = if (ctx.optionValue("retention")) |value| try parseFloat(value) else null,
    } } });
}
...
fn backupHandler(ctx: *th.Context) !void {
    if (ctx.args.len == 1) _ = try parseId(ctx.args[0]);
    try setRoute(ctx, .backup_cli);
}

fn restoreHandler(ctx: *th.Context) !void {
    if (!ctx.hasOption("dry-run") and !ctx.hasOption("yes")) return error.ConfirmationRequired;
    try setRoute(ctx, .backup_cli);
}
...
fn sackExportHandler(ctx: *th.Context) !void {
    _ = try parseId(ctx.args[0]);
    _ = try requireText(ctx.args[1]);
    try setRoute(ctx, .rich_cli);
}

fn sackImportHandler(ctx: *th.Context) !void {
    _ = try requireText(ctx.args[0]);
    try setRoute(ctx, .rich_cli);
}
...
const help_command: th.Command = .{
    .name = "help",
    .summary = "Show help for a command group",
    .handler = helpHandler,
    .args = .{ .positionals = &.{.{ .name = "topic", .required = false }} },
};
...
const deck_command: th.Command = .{
    ...
    .children = &.{ &deck_add_command, &deck_rename_command, &deck_delete_command, &deck_export_command, &deck_import_command },
};

const nut_export_command: th.Command = .{
    .name = "export",
    .summary = "Export a .nut deck",
    .handler = nutExportHandler,
    .args = .{ .positionals = &.{.{ .name = "deck-id" }} },
};
...
const card_add_command: th.Command = .{
    ...
    .handler = cardAddHandler,
    .args = .{ .positionals = &.{ .{ .name = "deck-id" }, .{ .name = "question" }, .{ .name = "answer" } } },
};

const card_edit_command: th.Command = .{
    .name = "edit",
    .summary = "Edit a card",
    .handler = cardEditHandler,
    .args = .{ .positionals = &.{ .{ .name = "card-id" }, .{ .name = "question" }, .{ .name = "answer" } } },
};

const card_delete_command: th.Command = .{
    .name = "delete",
    .summary = "Delete a card",
    .handler = cardDeleteHandler,
...
const study_command: th.Command = .{
    ...
    .args = .{ .positionals = &.{.{ .name = "deck-id" }} },
    .options = &.{
        .{ .long = "new-limit", .kind = .value, .value_type = .integer, .value_name = "count", .summary = "Limit new cards" },
        .{ .long = "order", .kind = .value, .value_type = .choice, .value_name = "order", .choices = &.{ "due", "reviews-first", "new-first" }, .summary = "Study queue order" },
...
const fsrs_simulate_command: th.Command = .{
    ...
    .handler = fsrsSimulateHandler,
    .args = .{ .exact = 0 },
    .options = &.{.{ .long = "retention", .kind = .value, .value_type = .float, .value_name = "0..1", .summary = "Desired retention" }},
};

const fsrs_retention_command: th.Command = .{
    .name = "retention",
    .summary = "Show current retention",
    .handler = fsrsRetentionHandler,
    .args = .{ .exact = 0 },
};
...
const fsrs_command: th.Command = .{
    .name = "fsrs",
    .summary = "FSRS tools",
    .children = &.{ &fsrs_optimize_command, &fsrs_evaluate_command, &fsrs_simulate_command, &fsrs_retention_command },
};

const scheduler_list_command: th.Command = .{
    .name = "list",
    .summary = "List scheduler configuration",
    .handler = schedulerListHandler,
    .args = .{ .exact = 0 },
};
...
fn helpFor(command: *const th.Command) Help {
    ...
    if (command == &nuts_command or command == &nut_command or command == &nut_export_command or command == &nut_import_command) return .{ .core = .nut };
    ...
    if (command == &media_command or command == &media_add_command or command == &sack_command or command == &sack_export_command or command == &sack_import_command) return .rich;
    return .general;
}
...
pub fn parse(allocator: std.mem.Allocator, argv: []const []const u8) !Route {
    ...
    const selection = switch (th.resolveWithOptions(&root_command, args, .{
