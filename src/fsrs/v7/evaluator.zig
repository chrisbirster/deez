...
fn newestTimestamp(histories: []const []const HistoryEntry) ?time.TimestampMs {
    var newest: ?time.TimestampMs = null;
    for (histories) |history| {
        if (history.len == 0) continue;
        const candidate = history[history.len - 1].reviewed_at_ms;
        if (newest == null or candidate > newest.?) newest = candidate;
    }
    return newest;
}
...
pub fn evaluate(
    ...
    const newest_ms = newestTimestamp(histories) orelse return .{};
...
test "evaluation predicts review history" {
    ...
    try std.testing.expect(metrics.log_loss > 0);
    try std.testing.expect(metrics.log_loss < 10);
